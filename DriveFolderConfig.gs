 ...
 *
 * Variaveis esperadas nas propriedades do script conforme uso real:
 *   INPUT_FOLDER_ID  - arquivos fonte/importacao.
 *   OUTPUT_FOLDER_ID - relatorios, payloads, PDFs, CSVs e artefatos gerados.
 *   BACKUP_FOLDER_ID - backups e snapshots restauraveis.
 *
...
var DRIVE_FOLDER_ALIASES = {
  ...
  OUTPUT_FOLDER_ID: ['DRIVE_OUTPUT_FOLDER_ID', 'REPORT_FOLDER_ID', 'DRIVE_FOLDER_ID', 'FOLDER_ID', 'PASTA_DESTINO_ID'],
...
function validateDriveFolder_(key, folderId) {
  var result = { key: key, folderId: folderId || '', configured: !!folderId, ok: false, name: '', url: '', error: '' };
...
function createFileInConfiguredFolder_(folder, fileName, content, mimeType) {
  try {
    ...
    return { ok: true, id: file.getId(), name: file.getName(), url: file.getUrl(), folderId: folder.getId(), generatedAt: new Date().toISOString() };
  } catch (error) {
